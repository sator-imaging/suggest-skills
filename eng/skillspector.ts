...
const SCAN_SORT_ORDER: Record<string, number> = {
  "Clone failed": 0,
  "Failed": 1,
  "Timed out": 2,
  "Succeeded": 3,
...
function compareResults(a: ScanResult, b: ScanResult): number {
  const scanCmp = (SCAN_SORT_ORDER[scanLabel(a)] ?? 99) - (SCAN_SORT_ORDER[scanLabel(b)] ?? 99);
  if (scanCmp !== 0) return scanCmp;
  return riskSortValue(b) - riskSortValue(a);
}

function repoStats(results: ScanResult[]): string {
  const ok = results.filter((r) => r.status === "OK");
  const failed = results.filter((r) => r.status === "FAILED");
  const cloneFailed = results.filter((r) => r.status === "CLONE_FAILED");
  const timedOut = results.filter((r) => r.status === "TIMEOUT");
  return `**Scanned: ${results.length} | Succeeded: ${ok.length} | Failed: ${failed.length} | Clone failed: ${cloneFailed.length} | Timed out: ${timedOut.length}**`;
}
...
function writeReport(results: ScanResult[]) {
  ...
  for (const repo of [...byRepo.keys()].sort()) {
    ...
    lines.push(`### ${repo}`);
    lines.push(repoStats(repoResults));
    lines.push("");
    lines.push("| Scan | Risk | Skill |");
    lines.push("|------|------|-------|");

    for (const r of [...repoResults].sort(compareResults)) {
